...
/**
 * 현재 선택된 탭 아이템 name을 반환한다.
 * @param {cpr.core.AppInstance} app 앱인스턴스
 * @param {#tabfolder} psTabId 탭 ID
 * @return {String} 탭 아이템 Name (탭 아이템 고유의 값)
 */
TabKit.prototype.getSelectedName = function(app, psTabId) { // 2021-01-05 류다은 추가
	/** @type cpr.controls.TabFolder */
	var vcTab = app.lookup(psTabId);
	var vcTabItem = vcTab.getSelectedTabItem();
	
	return vcTabItem ? vcTabItem.name : "";
}
 ...
 * @param {(pbSuccess:Boolean)=>Void} successCallback 서브미션 후 콜백 메소드
